...
products.map(rec => (NullWritable.get(), rec)).saveAsSequenceFile("/user/paslechoix/products_seq")


Below failed:
import org.apache.hadoop.io._
ordersDF.rdd.map(rec => (NullWritable.get(), rec)).saveAsSequenceFile("/user/paslechoix/orders_seq")
