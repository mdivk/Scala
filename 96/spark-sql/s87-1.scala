...
val products = sc.textFile("sparksql87/product.csv") 
...
val prdDF = prdRDD.tODF("productlD","productCode","name","quantity","price","supplierid") 
val supDF = supRDD.tODF("Supplierid","name","phone") 
val prdsupDF = prdsupRDD.tODF("productlD","supplierID") 
...
val results1 = sqlContext.sql( "SELECT products.name, price, suppliers.name as sup_name FROM products JOIN suppliers ON products.supplierID = suppliers.supplierID WHERE PRICE <0.6")
results1.show() 
